...
def main_menu(message):
    ...
    key.row("Мой профиль", "События", "Найти похожих пользователей")
...
def profile_events_match_btns(message):
    if message.text == "Мой профиль":
        ...
        if get_user_tb_column_val(message.from_user.username, "info_stage") == 6:
            ...
            keyboard = telebot.types.ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True)
            keyboard.row("Показать еще пользователей", "Вернуться в меню")
...
def make_users_match(username, chat_id, start=config.start, top_n=config.top_n):
    ...
    if challengers:
        ...
        keyboard.row("Показать еще пользователей", "В главное меню")
        bot.send_message(chat_id, text="По-моему, эти чуваки могли бы составить тебе компанию:", reply_markup=keyboard)
...
def match_handler(message):
    if message.text == "Показать еще пользователей":
        ...
        keyboard.row("Показать еще пользователей", "Вернуться в меню")
        send = bot.send_message(message.chat.id, text="Чекай чуваков", reply_markup=keyboard)
        bot.register_next_step_handler(send, match_handler)
...
def make_event_match(username, chat_id, event_day=None, event_time=None, start=config.start):
    possible_options = get_all_events_by_day(event_day, username)
    if possible_options:
        best_options = event_match(username, event_day, event_time)
        try:
            best_options = best_options[start:start + config.top_n]
        except IndexError:
            try:
